/* Class282 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

final class Class282
{
    static int anInt3651;
    static boolean aBoolean3652;
    static int anInt3653;
    static HashTable aClass356_3654;
    static int anInt3655 = 0;
    
    static final void method2109(int i) {
	anInt3653++;
	Class348_Sub42_Sub18 class348_sub42_sub18
	    = ((Class348_Sub42_Sub18)
	       Class348_Sub40_Sub17.aClass262_9240.getFirst(4));
	if (i != 512)
	    method2110(-125, 79, 70);
	for (/**/; class348_sub42_sub18 != null;
	     class348_sub42_sub18
		 = (Class348_Sub42_Sub18) Class348_Sub40_Sub17
					      .aClass262_9240
					      .nextForward((byte) 56)) {
	    Class318_Sub1_Sub3_Sub5 class318_sub1_sub3_sub5
		= (((Class348_Sub42_Sub18) class348_sub42_sub18)
		   .aClass318_Sub1_Sub3_Sub5_9682);
	    if ((((Class318_Sub1_Sub3_Sub5) class318_sub1_sub3_sub5).anInt10392
		 ^ 0xffffffff)
		> (Class367_Sub11.logicCycle ^ 0xffffffff)) {
		class348_sub42_sub18.removeNode();
		class318_sub1_sub3_sub5.method2472(-2159);
	    } else if ((Class367_Sub11.logicCycle ^ 0xffffffff)
		       <= ((((Class318_Sub1_Sub3_Sub5) class318_sub1_sub3_sub5)
			    .anInt10411)
			   ^ 0xffffffff)) {
		class318_sub1_sub3_sub5.method2474((byte) 121);
		if ((((Class318_Sub1_Sub3_Sub5) class318_sub1_sub3_sub5)
		     .anInt10412)
		    > 0) {
		    Class348_Sub22 class348_sub22
			= ((Class348_Sub22)
			   (aClass356_3654.get
			    ((long) (-1
				     + ((Class318_Sub1_Sub3_Sub5)
					class318_sub1_sub3_sub5).anInt10412))));
		    if (class348_sub22 != null) {
			Npc class318_sub1_sub3_sub3_sub1
			    = (((Class348_Sub22) class348_sub22)
			       .aClass318_Sub1_Sub3_Sub3_Sub1_6859);
			if (((((Class318_Sub1) class318_sub1_sub3_sub3_sub1)
			      .anInt6377)
			     ^ 0xffffffff) <= -1
			    && (Class367_Sub4.anInt7319 * 512
				> ((Class318_Sub1)
				   class318_sub1_sub3_sub3_sub1).anInt6377)
			    && ((((Class318_Sub1) class318_sub1_sub3_sub3_sub1)
				 .anInt6388)
				^ 0xffffffff) <= -1
			    && (512 * Class348_Sub40_Sub3.anInt9109
				> ((Class318_Sub1)
				   class318_sub1_sub3_sub3_sub1).anInt6388))
			    class318_sub1_sub3_sub5.method2471
				((byte) -103,
				 ((Class275.method2064
				   (((Class318_Sub1)
				     class318_sub1_sub3_sub3_sub1).anInt6377,
				    (((Class318_Sub1) class318_sub1_sub3_sub5)
				     .aByte6381),
				    11219,
				    ((Class318_Sub1)
				     class318_sub1_sub3_sub3_sub1).anInt6388))
				  + -((Class318_Sub1_Sub3_Sub5)
				      class318_sub1_sub3_sub5).anInt10366),
				 ((Class318_Sub1)
				  class318_sub1_sub3_sub3_sub1).anInt6388,
				 Class367_Sub11.logicCycle,
				 ((Class318_Sub1)
				  class318_sub1_sub3_sub3_sub1).anInt6377);
		    }
		}
		if ((((Class318_Sub1_Sub3_Sub5) class318_sub1_sub3_sub5)
		     .anInt10412)
		    < 0) {
		    int i_0_
			= -(((Class318_Sub1_Sub3_Sub5) class318_sub1_sub3_sub5)
			    .anInt10412) - 1;
		    Player class318_sub1_sub3_sub3_sub2;
		    if (i_0_ == Class348_Sub42_Sub11.localPlayerId)
			class318_sub1_sub3_sub3_sub2
			    = Class132.localPlayer;
		    else
			class318_sub1_sub3_sub3_sub2
			    = (ClassicLoadingScreen.players
			       [i_0_]);
		    if (class318_sub1_sub3_sub3_sub2 != null
			&& (((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
			    .anInt6377) >= 0
			&& ((512 * Class367_Sub4.anInt7319 ^ 0xffffffff)
			    < ((((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
				.anInt6377)
			       ^ 0xffffffff))
			&& ((((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
			     .anInt6388)
			    ^ 0xffffffff) <= -1
			&& (((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
			    .anInt6388) < 512 * Class348_Sub40_Sub3.anInt9109)
			class318_sub1_sub3_sub5.method2471
			    ((byte) -103,
			     ((Class275.method2064
			       ((((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
				 .anInt6377),
				(((Class318_Sub1) class318_sub1_sub3_sub5)
				 .aByte6381),
				11219,
				(((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
				 .anInt6388)))
			      - ((Class318_Sub1_Sub3_Sub5)
				 class318_sub1_sub3_sub5).anInt10366),
			     (((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
			      .anInt6388),
			     Class367_Sub11.logicCycle,
			     (((Class318_Sub1) class318_sub1_sub3_sub3_sub2)
			      .anInt6377));
		}
		class318_sub1_sub3_sub5.method2475((byte) 75,
						   Class348_Sub51.anInt7267);
		Class89.method850(class318_sub1_sub3_sub5, true);
	    }
	}
    }
    
    static final Class318_Sub1_Sub2 method2110(int i, int i_1_, int i_2_) {
	Class357 class357
	    = Message.aClass357ArrayArrayArray2029[i][i_1_][i_2_];
	if (class357 == null)
	    return null;
	Class318_Sub1_Sub2 class318_sub1_sub2
	    = ((Class357) class357).aClass318_Sub1_Sub2_4408;
	((Class357) class357).aClass318_Sub1_Sub2_4408 = null;
	Class183.method1376(class318_sub1_sub2);
	return class318_sub1_sub2;
    }
    
    public static void method2111(byte i) {
	aClass356_3654 = null;
	if (i <= 56)
	    method2111((byte) -49);
    }
    
    static final boolean method2112(int i, int i_3_) {
	anInt3651++;
	if (i <= 121)
	    return false;
	if (i_3_ != 3 && i_3_ != 4)
	    return false;
	return true;
    }
    
    static {
	aClass356_3654 = new HashTable(64);
    }
}
