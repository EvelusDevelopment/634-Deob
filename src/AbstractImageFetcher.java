/* AbstractImageFetcher - Decompiled by JODE
...
abstract class AbstractImageFetcher extends Node
{
    ...
    static OutgoingPacket aClass351_6925;
    ...
    abstract void initialize(Canvas canvas, int i, int i_1_, int i_2_);
    ...
    abstract void drawImage(Graphics graphics, int i_41_, int i, int i_38_, int i_43_, int i_42_, int i_39_);
    ...
    static final void readScriptSettings(ByteBuffer buffer, byte i) {
	anInt6914++;
	if (((((ByteBuffer) buffer).payload.length
	      - ((ByteBuffer) buffer).position)
	     ^ 0xffffffff)
	    <= -2) {
	    int i_89_ = buffer.getUByte();
	    if (i_89_ >= 0 && (i_89_ ^ 0xffffffff) >= -2
		&& ((((ByteBuffer) buffer).payload.length
		     - ((ByteBuffer) buffer).position)
		    ^ 0xffffffff) <= -3) {
		if (i != -40)
		    method3012(13, -47, -27, 107, -90, (byte) -19, -61, 36);
		int i_90_ = buffer.getShort();
		if (i_90_ * 6 <= (-((ByteBuffer) buffer).position
				  + (((ByteBuffer) buffer)
				     .payload).length)) {
		    for (int i_91_ = 0; i_91_ < i_90_; i_91_++) {
			int id = buffer.getShort();
			int value = buffer.getDWord();
			if (((id ^ 0xffffffff)
			     > (ISAAC.globalScriptIntVars.length ^ 0xffffffff))
			    && Class286_Sub6.isCsVariableActive[id]
			    && ((((IntergerVarScriptSettingDefinition) Class239_Sub14.scriptGlobalIntArgsLoader
						 .getScriptSetting(id, -107))
				 .loadFromSettings) != '1'
				|| (value ^ 0xffffffff) <= 0 && value <= 1))
			    ISAAC.globalScriptIntVars[id] = value;
		    }
		}
	    }
	}
	...
	aBigInteger6921
		...
		16));
	aString6926 = null;
	aClass351_6925 = new OutgoingPacket(31, 1);
    }
}
