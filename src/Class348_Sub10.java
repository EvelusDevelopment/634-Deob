...
final class Class348_Sub10 extends Node
{
	...
	anInt6702 = class348_sub49.getDWord();
	anInt6716 = class348_sub49.getDWord();
	anInt6723 = class348_sub49.getDWord();
	anInt6717 = class348_sub49.getDWord();
	if (anInt6717 < 0) {
	    anInt6717 = anInt6717 ^ 0xffffffff;
	    aBoolean6728 = true;
	}
	int i = class348_sub49.getDWord();
	aByteArrayArray6703 = new byte[i][];
	for (int i_3_ = 0; i_3_ < i; i_3_++) {
	    int i_4_ = 0;
	    int i_5_;
	    do {
		i_5_ = class348_sub49.getUByte();
		i_4_ += i_5_;
	    } while (i_5_ >= 255);
	    byte[] is_6_ = new byte[i_4_];
	    class348_sub49.getBytes(is_6_, 0, i_4_);
	...
	if (!aBoolean6724) {
	    byte[] is = class45.loadArchiveChild(-1860, 0, 0);
	    if (is == null)
		return false;
	    method2790(is);
	}
    ...
    static final Class348_Sub10 method2795(IndexLoader class45, int i,
					   int i_115_) {
	if (!method2792(class45)) {
	    class45.isLoaded(i, i_115_);
	    return null;
	}
	byte[] is = class45.loadArchiveChild(-1860, i, i_115_);
	if (is == null)
	    return null;
	return new Class348_Sub10(is);
    }
