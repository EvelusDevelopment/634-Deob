...
final class Class179
{
    ...
    static final Object getDirectByteBuffer(byte[] is) {
	anInt2363++;
	if (is == null)
	    return null;
	if (is.length > 136 && !AnimationDefinition.aBoolean247) {
	    try {
		AbstractBuffer buffer = (AbstractBuffer) Class.forName("DirectByteBuffer").newInstance();
		buffer.put(is);
		return buffer;
	    } catch (Throwable throwable) {
		AnimationDefinition.aBoolean247 = true;
	    }
	}
	return is;
    }
    ...
    static final void method1360(String string, Signlink class297,
				 boolean bool, boolean bool_3_, int i) {
	try {
	    anInt2369++;
	    if (bool_3_) {
		if (Signlink.osName.startsWith("win")
		    && ((Signlink) class297).allowsFileOperations) {
		    String string_4_ = null;
		    if (Class93.loaderApplet != null)
			string_4_
			    = Class93.loaderApplet.getParameter("haveie6");
			...
			ModernLoadingScreen.aClass297_5017 = class297;
		...
		if (Signlink.osName.startsWith("mac")) {
		    ...
		    if (Class93.loaderApplet != null)
			string_5_
			    = Class93.loaderApplet.getParameter("havefirefox");
	...
	if (Class374.randomDatBufferedFile != null) {
	    try {
		Class374.randomDatBufferedFile.method789(0L, (byte) 59);
		Class374.randomDatBufferedFile.method783(i, 24, true,
						 (((ByteBuffer)
						   class348_sub49)
						  .payload));
