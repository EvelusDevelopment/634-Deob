...
final class FileOnDisk
{
    ...
    final int read(byte[] is, int i, byte i_0_, int i_1_)
	throws IOException {
	anInt3040++;
    ...
    final void write(byte i, int i_4_, int i_5_, byte[] is)
	throws IOException {
	anInt3043++;
	if (((long) i_5_ - -aLong3039 ^ 0xffffffffffffffffL) < (maximumSize ^ 0xffffffffffffffffL)) {
	    aRandomAccessFile3036.seek(maximumSize);
